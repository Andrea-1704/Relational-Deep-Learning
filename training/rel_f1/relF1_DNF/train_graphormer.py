...
# Opzionale (commentato): VGAE pretraining
...
def set_global_seed(seed: int, deterministic: bool = True):
    """Set all seeds consistently for reproducibility."""
...
def build_data_and_targets(device, target_col="did_not_finish"):
    """
    Load rel-f1, driver-dnf task (binary), build graph, and attach y + train_mask
    only for nodes with known training labels.
    """
    ...
    # Graph used for loaders and model construction
    ...
    labels_raw = train_df_raw[target_col].to_numpy()  # already binary 0/1
    ...
    # Full graph on device (useful for pos_weight)
...
def build_model(data_official, col_stats_official, device, channels=128):
    # Your settings: 2 layers, norm="batch_norm"
    model = Model(
        data=data_official,
...
def run_once(seed: int, device: torch.device, max_epochs: int = 500, use_vgae: bool = False):
    ...
    test_table = task.get_table("test", mask_input_cols=False)  # unmasked test
    ...
    # Optional: VGAE pretraining (OFF by default)
    # if use_vgae:
    #     tmp_loader = loader_dict_fn(
    #         batch_size=64, num_neighbours=32, data=data_official, task=task,
    ...
    early_stopping = EarlyStopping(
        ...
        delta=0.0,
        verbose=False,
        path=f"best_graphormer_dnf_seed{seed}.pt"
    )

    # Your Graphormer loader settings
...
def main():
    ...
    print("\n=== Summary over 5 seeds (GraphormerNew - driver-dnf) ===")
    for r in results:
