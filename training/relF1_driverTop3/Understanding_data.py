...
def train2():
    ...
    #print(f"{data_official.table_dict}")#no 
    ...
    driver_id = 0
    local_idx = global_to_local[driver_id]

    x = data_official["drivers"].x
    embedding = x[local_idx]

    encoder = HeteroEncoder(
        channels=128,
        node_to_col_names_dict={
            ntype: data_official[ntype].tf.col_names_dict
            for ntype in data_full.node_types
        },
    ...
    print(f"drivers id: {db_nuovo.table_dict['drivers'].df['driverId'].to_numpy()[:10]}")
    print(f"features primo driver: {db_nuovo.table_dict['drivers']}")
    #print(f"data official primo driver: {data_official['drivers']}")
    print(f"embeddings ricavate nodo 0: {embedding}")
    ...
    print(f"data full di driver è {data_full['drivers']}")
