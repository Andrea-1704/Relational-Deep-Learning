...
#ONLY IN THIS CASE IN WHICH THE LABEL IS FALSE/TRUE INSTEAD OF 0/1->CONVERSION:
target_vector_official = torch.full(
    (len(graph_driver_ids),),
    float("nan"),
    dtype=torch.float32
)
labels_np = binary_top3_labels_raw.astype("float32")
for i, driver_id in enumerate(driver_ids_raw):
    j = id_to_idx.get(driver_id)
    if j is not None:
        target_vector_official[j] = float(labels_np[i])
