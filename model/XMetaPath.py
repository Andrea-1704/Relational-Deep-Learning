...
class MetaPathGNNLayer(MessagePassing):
    ...
    def __init__(self, hidden_channels: int):
        ...
        self.w_0 = nn.Linear(hidden_channels, hidden_channels, bias=True)
        self.w_1 = nn.Linear(hidden_channels, hidden_channels, bias=True)
        self.gate = nn.Parameter(torch.tensor(0.5))
    ...
    def forward(
        ...
        )  # -> [#dst_loc, D] perché flow=source_to_target

        # normalizzazione su 'dst'
        row = edge_index[1]  # indici dst locali
        if edge_weight is None:
...
class MetaPathGNN(nn.Module):
    """
    Applica una meta-path (lista di hop (src, rel, dst)).
    - Verifica che la meta-path termini su 'drivers'.
    - Applica i layer in ordine inverso, aggiornando i 'dst' ad ogni hop.
    - Ritorna le embedding dei 'drivers' (tutti i drivers nel subgrafo passato).
    """
    def __init__(self, metapath: list[tuple[str, str, str]], hidden_channels: int, target_type: str = "drivers", dropout: float = 0.2):
        super().__init__()
        assert isinstance(metapath, (list, tuple)) and len(metapath) >= 1
        if metapath[-1][2] != target_type:
            raise ValueError(f"Metapath must end on '{target_type}', got '{metapath[-1][2]}'")

        self.metapath = metapath
        self.target_type = target_type
        self.dropout = dropout

        # un layer per ogni hop (stessa D in/out)
        self.convs = nn.ModuleList([MetaPathGNNLayer(hidden_channels) for _ in metapath])

        # proiezione finale (facoltativa): qui identità lineare
        self.out_proj = nn.Identity()

    @torch.no_grad()
    def _get_edge(self, edge_index_dict, key):
        # edge_index_dict chiave: (src, rel, dst)
        if key in edge_index_dict:
            return edge_index_dict[key]
        # fallback: prova anche senza 'rev_' o con trasposto
        src, rel, dst = key
        alt = (dst, f"rev_{rel}" if not rel.startswith("rev_") else rel[4:], src)
        if alt in edge_index_dict:
            # trasponi per mantenere (src,dst)
            return edge_index_dict[alt].flip(0)
        raise KeyError(f"Edge key {key} not found and no fallback available.")

    def forward(
        self,
        x_dict: dict[str, torch.Tensor],                 # features già in hidden_dim per ciascun tipo
        edge_index_dict: dict[tuple[str, str, str], torch.Tensor],
        node_time_dict: dict[str, torch.Tensor] | None = None,
        edge_weight_dict: dict[tuple[str, str, str], torch.Tensor] | None = None,
    ) -> torch.Tensor:
        # h_0 = x, ma teniamo copia "fissa" delle feature originali (detach)
        h_dict = {k: v for k, v in x_dict.items()}
        x0_dict = {k: v.detach() for k, v in x_dict.items()}

        # Applica in ordine inverso: r_L, ..., r_1
        for layer, (src, rel, dst) in zip(reversed(self.convs), reversed(self.metapath)):
            edge_index = self._get_edge(edge_index_dict, (src, rel, dst))

            # NOTA: qui usiamo TUTTI i nodi locali del subgrafo (seed-first garantito dal loader)
            h_src = h_dict[src]
            h_dst = h_dict[dst]
            x_dst_orig = x0_dict[dst]

            ew = None
            if edge_weight_dict is not None and (src, rel, dst) in edge_weight_dict:
                ew = edge_weight_dict[(src, rel, dst)]

            h_new = layer(h_src, h_dst, edge_index, x_dst_orig, edge_weight=ew)
            h_new = F.relu(h_new)
            h_new = F.dropout(h_new, p=self.dropout, training=self.training)
            h_dict[dst] = h_new

        # restituisce TUTTI i drivers locali (seed-first), lo slicing a B lo fa XMetaPath
        return self.out_proj(h_dict[self.target_type])


class XMetaPath(nn.Module):
    """
    Combina più MetaPathGNN con pesi apprendibili (softmax) e una testa MLP binaria.
    Ritorna logits [B], già allineati ai soli seed 'drivers' del batch.
    """
    def __init__(self, metapaths: list[list[tuple[str, str, str]]], hidden_channels: int = 128, dropout: float = 0.2):
        super().__init__()
        if len(metapaths) == 0:
            raise ValueError("Provide at least one meta-path.")
        # guard-rail: tutte devono terminare su 'drivers'
        for mp in metapaths:
            if mp[-1][2] != "drivers":
                raise ValueError(f"Metapath must end on 'drivers', got '{mp[-1][2]}' -> {mp}")

        self.metapath_models = nn.ModuleList([
            MetaPathGNN(mp, hidden_channels, target_type="drivers", dropout=dropout) for mp in metapaths
        ])
        self.path_logits = nn.Parameter(torch.zeros(len(metapaths)))  # pesi apprendibili tra path

        # testa binaria; input = hidden_channels dopo combinazione
    ...
    def forward(self, batch, entity_table: str = "drivers"):
        x_dict = batch.x_dict
        edge_index_dict = batch.edge_index_dict
        node_time_dict = getattr(batch, "time_dict", None)

        # numero di seed nel mini-batch: seed-first (PyG/RelBench)
        if hasattr(batch[entity_table], "y"):
            B = batch[entity_table].y.size(0)
        elif hasattr(batch[entity_table], "batch_size"):
            B = int(batch[entity_table].batch_size)  # a volte esposto così
        else:
            raise RuntimeError("Cannot infer batch size (B) for entity_table.")

        per_path = []
        for model in self.metapath_models:
            # embedding di TUTTI i drivers locali; poi taglio ai primi B (seed-first)
            h = model(x_dict, edge_index_dict, node_time_dict=node_time_dict)  # [N_local_drivers, D]
            per_path.append(h[:B])  # [B, D]

        H = torch.stack(per_path, dim=1)  # [B, M, D]
        alpha = torch.softmax(self.path_logits, dim=0)  # [M]
        Hc = (H * alpha.view(1, -1, 1)).sum(dim=1)      # [B, D] combinazione pesata

        logits = self.regressor(Hc).squeeze(-1)         # [B]
        return logits
